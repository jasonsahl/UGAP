...
def run_single_loop(assembler,forward_path,reverse_path,name,error_corrector,processors,keep,start_path,reduce,careful,UGAP_PATH,
    ...
    if os.path.isfile("%s.F.paired.fastq.gz" % name):
        ...
        if sample_type == "PE":
            ...
            subprocess.check_call("bbduk.sh k=21 in=%s.F.tmp.fastq.gz in2=%s.R.tmp.fastq.gz ref=%s/bin/illumina_adapters_all.fasta out=%s.F.paired.fastq.gz out2=%s.R.paired.fastq.gz minlen=%s overwrite=true" % (name,name,UGAP_PATH,name,name,length), shell=True)
        elif sample_type == "SE":
            ...
            subprocess.check_call("bbduk.sh k=21 in=%s.F.tmp.fastq.gz ref=%s/bin/illumina_adapters_all.fasta out=%s.F.paired.fastq.gz minlen=%s overwrite=true" % (name,UGAP_PATH,name,length), shell=True)
        if phiX_filter == "T":
            try:
                if sample_type == "PE":
