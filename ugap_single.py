...
def find_missing_coverages(depth, merged, lengths):
    all_ids = {}
...
def run_single_loop(forward_path,reverse_path,name,error_corrector,processors,keep,start_path,reduce,careful,UGAP_PATH, TRIM_PATH, PICARD_PATH, PILON_PATH, GATK_PATH, blast_nt, cov_cutoff):
    ...
    doc("coverage.out", "genome_size.txt", name, 3)
    ...
    if "NULL" not in blast_nt:
        ...
        merge_blast_with_coverages("blast.uniques", "%s_3_depth.txt" % name, lengths)
        ...
        find_missing_coverages("%s_3_depth.txt" % name, "%s/UGAP_assembly_results/%s_blast_depth_merged.txt" % (start_path, name), lengths)
